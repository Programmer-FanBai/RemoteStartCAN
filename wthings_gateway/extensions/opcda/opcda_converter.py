...
class CustomOpcdaUplinkConverter(Converter):
    ...
    def __init__(self, config):
        ...
        if self.__config.get("attributes") is not None:
            for attribute in self.__config["attributes"]:
                ...
                self.full_path_key_maps.append({attribute_full_path:{"key": attribute["key"], "uploadType": "attributes", "type": attribute["type"]}})
        ...
        if self.__config.get("timeseries") is not None:
            for timeserie in self.__config["timeseries"]:
                ...
                self.full_path_key_maps.append({timeserie_full_path : {"key": timeserie["key"], "uploadType": "telemetry", "type": timeserie["type"]}})
    ...
    def convert(self, config, data):
        ...
        for i, v in enumerate(data):
            ...
            for full_path_key_map in self.full_path_key_maps:
                things_type_key = full_path_key_map.get(name, None)
                if things_type_key:
