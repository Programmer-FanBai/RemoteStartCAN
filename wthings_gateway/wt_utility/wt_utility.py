...
class WTUtility:
    ...
    def check_and_import(extension_type, module_name):
        if WTUtility.loaded_extensions.get(extension_type + module_name) is None:
            extensions_paths = (path.abspath(path.dirname(path.dirname(__file__)) + '/connectors/'.replace('/', path.sep) + extension_type.lower()),
                                '/var/lib/wthings_gateway/extensions/'.replace('/', path.sep) + extension_type.lower(),
                                ...
                                try:
                                    ...
                                    for extension_class in getmembers(module, isclass):
                                        if module_name in extension_class:
                                            ...
                                            WTUtility.loaded_extensions[extension_type + module_name] = extension_class[1]
            ...
            log.debug("Class %s found in WTUtility buffer.", module_name)
            return WTUtility.loaded_extensions[extension_type + module_name]
