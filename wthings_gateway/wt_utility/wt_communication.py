...
if __name__ == '__main__':
    plc_1500 = SiemensS7Net(SiemensPLCS.S1500, "10.25.157.92")
    result = plc_1500.ConnectServer()
    if result.IsSuccess:
        print("plc_1500 10.25.157.92 连接成功")
        sensor_exit_t = plc_1500.ReadFloat("DB904.0")
        print("入口温度=", sensor_exit_t.Content)
        # sensor_exit_h = plc_1500.Read("DB904", 1)
        # plc_1500.ConnectClose()
        sensor_exit_h = plc_1500.ReadFloat("DB904.4")
        print("入口湿度=", sensor_exit_h.Content)
    else:
        print("连接失败")
    plc_1500 = SiemensS7Net(SiemensPLCS.S1500, "10.25.157.90")
    result = plc_1500.ConnectServer()
    if result.IsSuccess:
        print("plc_1500 10.25.157.90 连接成功 ")
        sensor_exit_t = plc_1500.ReadInt16("DB1320.2")
        print("心跳计数=", sensor_exit_t.Content)
        sensor_exit_h = plc_1500.ReadFloat("DB902.108")
        print("电泳循环温度=", sensor_exit_h.Content)
    else:
        print("连接失败")

    plc_300 = SiemensS7Net(SiemensPLCS.S300, "10.25.157.81")
    plc_300.SetSlotAndRack(0, 2)
    result = plc_300.ConnectServer()
    if result.IsSuccess:
        print("plc_300 10.25.157.81 连接成功 ")
        sensor_exit_t = plc_300.ReadInt16("DB211.14")
        print("STQ输调漆喷房左侧开始累计喷枪时间小时干式自动右=", sensor_exit_t.Content)
        sensor_exit_t = plc_300.ReadInt32("DB5500.176")
        print("CV01位置值 单位mm=", sensor_exit_t.Content)
    else:
        print("连接失败")
