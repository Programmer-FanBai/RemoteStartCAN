    ...
    def convert(self, config, data):
        ...
        try:
            if self.__config['converter'].get("deviceNameJsonExpression") is not None:
                dict_result["deviceName"] = WTUtility.get_value(self.__config['converter'].get("deviceNameJsonExpression"), data, expression_instead_none=True)
            else:
            ...
            if self.__config['converter'].get("deviceTypeJsonExpression") is not None:
                dict_result["deviceType"] = WTUtility.get_value(self.__config['converter'].get("deviceTypeJsonExpression"), data, expression_instead_none=True)
            else:
        ...
        try:
            for datatype in self.__datatypes:
                ...
                for datatype_object_config in self.__config["converter"].get(datatype, []):
                    datatype_object_config_key = WTUtility.get_value(datatype_object_config["key"], data, datatype_object_config["type"], expression_instead_none=True)
                    datatype_object_config_value = WTUtility.get_value(datatype_object_config["value"], data, datatype_object_config["type"])
