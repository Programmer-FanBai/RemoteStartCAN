...
class BytesModbusUplinkConverter(ModbusConverter):
            ...
            for tag in data[config_data]:
                try:
                    ...
                    if not isinstance(response, ModbusIOException):
                        ...
                        elif configuration["functionCode"] in [3, 4, "3", "4"]:
                            decoder = None
                            registers = response.registers
                            log.debug("Tag: %s Config: %s registers: %s", tag, str(configuration), str(registers))
                            try:
                                decoder = BinaryPayloadDecoder.fromRegisters(registers,byteorder=Endian.Big, wordorder=endian_order)
                            except TypeError:
    ...
    def __decode_from_registers(decoder, configuration):
        ...
        if lower_type == "string":
            ...
            if lower_type == "bit":
                result = decoder_functions["16uint"]()
                result = "0" * (16 - len(bin(result)[2:])) + str(bin(result)[2:])
                decoded = bool(int(result[configuration.get("bit", 0)]))
            else:
                decoded = decoder_functions[lower_type]()
        elif lower_type in ['short', 'int', 'long', 'integer']:
