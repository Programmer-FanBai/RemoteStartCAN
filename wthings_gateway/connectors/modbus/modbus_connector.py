...
try:
    ...
    WTUtility.install_package("pymodbus", ">=2.3.0")
    from pymodbus.constants import Defaults
...
class ModbusConnector(Connector, threading.Thread):
    ...
    def open(self):
        self.__stopped = False
        self.start()
        log.info("Starting Modbus connector name:{name}".format(name=self.getName()))

    def run(self):
        while not self.__master.connect() and not self.__stopped:
            time.sleep(5)
            log.warning("Modbus trying reconnect to %s", self.__config.get("host", self.__config.get("port")))
        log.info("Modbus connected.name:{name}".format(name=self.getName()))
    ...
    def __function_to_device(self, config, unit_id):
        ...
        result = None
        # log.error("config: {config}".format(config=config))
        if function_code in (1, 2, 3, 4, "3", "4"):
...
if __name__ == '__main__':
    ...
    # registers = [0x3e12, 0x702f]
        print("registers=", hex(registers[0]), hex(registers[1]))
        # ABCD
        decoder = BinaryPayloadDecoder.fromRegisters(registers, byteorder=Endian.Big, wordorder=Endian.Big)
        print("decoder=", decoder.decode_32bit_float(), "3E12702F")
        # CDAB
        decoder1 = BinaryPayloadDecoder.fromRegisters(registers, byteorder=Endian.Big, wordorder=Endian.Little)
        print("decoder1=", decoder1.decode_32bit_float(), "702F3E12")
        # DCBA
        decoder2 = BinaryPayloadDecoder.fromRegisters(registers, byteorder=Endian.Little, wordorder=Endian.Little)
        print("decoder2=", decoder2.decode_32bit_float(), "2F70123e")
        # BADC
        decoder3 = BinaryPayloadDecoder.fromRegisters(registers, byteorder=Endian.Little, wordorder=Endian.Big)
        print("decoder3=", decoder3.decode_32bit_float(), "123E2F70")
