...
from wthings_gateway.connectors.odbc.odbc_converter import OdbcConverter
            ...
            try:
                if isinstance(config_item, str):
                    ...
                    if "column" in config_item:
                        ...
                        log.error("Failed to convert SQL data to WT format: no column/value configuration item")
                else:
                    log.error("Failed to convert SQL data to WT format: unexpected configuration type '%s'",
                              type(config_item))
            except Exception as e:
                log.error("Failed to convert SQL data to WT format: %s", str(e))
