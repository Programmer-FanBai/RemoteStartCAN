...
try:
    ...
    WTUtility.install_package("pyodbc")
    import pyodbc

from wthings_gateway.connectors.odbc.odbc_uplink_converter import OdbcUplinkConverter
...
class OdbcConnector(Connector, Thread):
    ...
    def __init__(self, gateway, config, connector_type):
        ...
        self.__converter = OdbcUplinkConverter() if not self.__config.get("converter", "") else \
            WTUtility.check_and_import(self.__connector_type, self.__config["converter"])
    ...
    def __check_and_send(self, device_name, device_type, new_data):
        ...
        for wt_key in to_send.keys():
            for key, new_value in new_data[wt_key].items():
                if not send_on_change or self.__devices[device_name][wt_key].get(key, None) != new_value:
                    self.__devices[device_name][wt_key][key] = new_value
        ...
        if to_send["attributes"] or to_send["telemetry"]:
            ...
            log.debug("[%s] Pushing to WT server '%s' device data: %s", self.get_name(), device_name, to_send)
    ...
    def __resolve_iterator_file(self):
        ...
        try:
            ...
            # https://wthings.io/docs/iot-gateway/config/odbc/#subsection-iterator
