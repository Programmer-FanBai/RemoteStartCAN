...
from wthings_gateway.connectors.rest.rest_converter import RESTConverter, log
    ...
    def convert(self, config, data):
        ...
        try:
            if self.__config.get("deviceNameExpression") is not None:
                dict_result["deviceName"] = WTUtility.get_value(self.__config.get("deviceNameExpression"), data, expression_instead_none=True)
            else:
            ...
            if self.__config.get("deviceTypeExpression") is not None:
                dict_result["deviceType"] = WTUtility.get_value(self.__config.get("deviceTypeExpression"), data, expression_instead_none=True)
            else:
        ...
        try:
            for datatype in datatypes:
                ...
                for datatype_config in self.__config.get(datatype, []):
                    value = WTUtility.get_value(datatype_config["value"], data, datatype_config["type"], expression_instead_none=True)
                    value_tag = WTUtility.get_value(datatype_config["value"], data, datatype_config["type"], get_tag=True)
                    key = WTUtility.get_value(datatype_config["key"], data, datatype_config["type"], expression_instead_none=True)
                    key_tag = WTUtility.get_value(datatype_config["key"], data, get_tag=True)
