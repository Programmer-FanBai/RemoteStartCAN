...
try:
    ...
    WTUtility.install_package("bacpypes", ">=0.18.0")
    from bacpypes.core import run, stop
...
from wthings_gateway.connectors.bacnet.bacnet_utilities.wt_gateway_bacnet_application import BACnetApplication
...
class BACnetConnector(Thread, Connector):
    def __init__(self, gateway, config, connector_type):
        ...
        self._application = BACnetApplication(self, self.__config)
        ...
        self.default_converters = {"uplink_converter": WTUtility.check_and_import(self.__connector_type, "BACnetUplinkConverter"),
                                     "downlink_converter": WTUtility.check_and_import(self.__connector_type, "BACnetDownlinkConverter")}
            ...
            for datatype_config in device.get(datatype, []):
                try:
                    for converter_type in self.default_converters:
                        converter_object = self.default_converters[converter_type] if datatype_config.get("class") is None else WTUtility.check_and_import(self.__connector_type, device.get("class"))
        ...
        if self.__devices_address_name.get(data["address"]) is None:
            for device in self.__config_devices:
                try:
                    ...
                    device_name_tag = WTUtility.get_value(device["deviceName"], get_tag=True)
