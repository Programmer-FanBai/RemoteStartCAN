...
class MqttConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        ...
        self.__gateway = gateway                 # Reference to WT Gateway
        ...
        # RPCs, i.e., remote procedure calls (WaveletThings towards devices) handlers
        self.load_handlers('serverSideRpc', mandatory_keys['serverSideRpc'], self.__server_side_rpc)

        # Connect requests, i.e., telling WaveletThings that a device is online even if it does not post telemetry
        self.load_handlers('connectRequests', mandatory_keys['connectRequests'], self.__connect_requests)

        # Disconnect requests, i.e., telling WaveletThings that a device is offline even if keep-alive has not expired yet
        self.load_handlers('disconnectRequests', mandatory_keys['disconnectRequests'], self.__disconnect_requests)

        # Shared attributes direct requests, i.e., asking WaveletThings for some shared attribute value
        self.load_handlers('attributeRequests', mandatory_keys['attributeRequests'], self.__attribute_requests)

        # Attributes updates requests, i.e., asking WaveletThings to send updates about an attribute
        ...
        if result_code == 0:
            ...
            for mapping in self.__mapping:
                try:
                    ...
                    module = WTUtility.check_and_import(self._connector_type, converter_class_name)
                    if module:
                    ...
                    regex_topic = WTUtility.topic_to_regex(mapping["topicFilter"])
            ...
            for request in [entry for entry in self.__connect_requests if entry is not None]:
                ...
                topic_filter = WTUtility.topic_to_regex(request.get("topicFilter"))
    ...
    def _on_message(self, client, userdata, message):
        ...
        if topic_handlers:
            ...
            # may produce more than one message towards WaveletThings. This also means that I cannot return after
        ...
        if topic_handlers:
            for topic in topic_handlers:
                ...
                if handler.get("deviceNameTopicExpression"):
                    ...
                    found_device_name = WTUtility.get_value(handler["deviceNameJsonExpression"], content)
                ...
                if handler.get("deviceTypeTopicExpression"):
                    ...
                    found_device_type = WTUtility.get_value(handler["deviceTypeJsonExpression"], content)

                if found_device_name is None:
                    self.__log.error("Device name missing from connection request")
                    continue

                # Note: device must be added even if it is already known locally: else WaveletThings
            ...
            try:
                for topic in topic_handlers:
                    ...
                    if handler.get("attributeNameTopicExpression"):
                        ...
                        found_attribute_name = WTUtility.get_value(handler["attributeNameJsonExpression"], content)
                    ...
                    self.__gateway.wt_client.client.gw_request_shared_attributes(
                        found_device_name,
