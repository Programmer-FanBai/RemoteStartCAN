...
from socket import gethoswtyname
...
try:
    ...
    WTUtility.install_package("puresnmp")
    import puresnmp
...
class SNMPConnector(Connector, Thread):
    ...
    def __fill_converters(self):
        try:
            for device in self.__devices:
                device["uplink_converter"] = WTUtility.check_and_import("snmp", device.get('converter', self._default_converters["uplink"]))(device)
                device["downlink_converter"] = WTUtility.check_and_import("snmp", device.get('converter', self._default_converters["downlink"]))(device)
        except Exception as e:
            log.exception(e)

    @staticmethod
    def __get_common_parameters(device):
        return {"ip": gethoswtyname(device["ip"]),
