    ...
    def convert(self, config, data):
        try:
            ...
            if not isinstance(data, dict) or not data:
                log.error("Failed to convert WT data to CAN payload: data is empty or not a dictionary")
                return
            ...
            if config.get("dataExpression", ""):
                ...
                log.error("Failed to convert WT data to CAN payload: no `value` or `dataExpression` property")
                return
            ...
            log.error("Failed to convert WT data to CAN payload: %s", str(e))
            return
