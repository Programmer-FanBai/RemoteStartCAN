...
try:
    ...
    WTUtility.install_package("python-can")
    from can import Notifier, BufferedReader, Message, CanError, ThreadSafeBus

from wthings_gateway.connectors.can.bytes_can_downlink_converter import BytesCanDownlinkConverter
from wthings_gateway.connectors.can.bytes_can_uplink_converter import BytesCanUplinkConverter
        ...
        for device_config in config.get("devices"):
            ...
            if "attributeUpdates" in device_config and device_config["attributeUpdates"]:
                ...
                for attribute_config in device_config["attributeUpdates"]:
                    ...
                    attribute_name = attribute_config.get("attributeOnWaveletThings") or attribute_config.get("attribute")
                ...
                for msg_config in device_config[config_key]:
                    ...
                    if "polling" in msg_config:
                        try:
                            ...
                            polling_config["key"] = wt_key  # Just for logging
                            ...
                            log.warning("[%s] Ignore '%s' %s polling configuration, wrong CAN data: %s",
                                        self.get_name(), wt_key, config_key, str(e))
                            continue
        ...
        if config is None:
            ...
            if need_uplink:
                ...
                return BytesCanUplinkConverter() if uplink is None \
                    else WTUtility.check_and_import(self.__connector_type, uplink)
            else:
                downlink = config.get("downlink")
                return BytesCanDownlinkConverter() if downlink is None \
                    else WTUtility.check_and_import(self.__connector_type, downlink)
