...
class WTClient(threading.Thread):
    ...
    def _on_connect(self, client, userdata, flags, result_code, *extra_params):
        log.debug('WT client %s connected to WaveletThings', str(client))
        if result_code == 0:
    ...
    def _on_disconnect(self, client, userdata, result_code):
        ...
        if self.client._client != client:
            log.info("WT client %s has been disconnected. Current client for connection is: %s", str(client), str(self.client._client))
            client.disconnect()
            client.loop_stop()
        else:
            self.__is_connected = False
        ...
        try:
            while not self.client.is_connected() and not self.__stopped:
                if not self.__paused:
                    if self.__stopped:
                        break
                    log.debug("connecting to WaveletThings")
                    try:
