...
class WTGatewayService:
    ...
    def __init__(self, config_file=None):
        ...
        if config_file is None:
            config_file = path.dirname(path.dirname(path.abspath(__file__))) + '/config/wt_gateway.yaml'.replace('/', path.sep)
        ...
        log.info("WaveletThings IoT gateway version: %s", self.version)
        ...
        if logging_error is not None:
            self.wt_client.client.send_telemetry({"ts": time()*1000, "values": {"LOGS": "Logging loading exception, logs.conf is wrong: %s" % (str(logging_error), )}})
            WTLoggerHandler.set_default_handler()
        self.counter = 0
        ...
        self._send_thread = Thread(target=self.__read_data_from_storage, daemon=True,
                                   name="Send data to Waveletthings Thread")
        ...
        try:
            ...
            while not self.stopped:
                cur_time = time()*1000
                if not self.wt_client.is_connected() and self.__subscribed_to_rpc_topics:
                    self.__subscribed_to_rpc_topics = False
                if self.wt_client.is_connected() and not self.__subscribed_to_rpc_topics:
                ...
                if (self.__rpc_requests_in_progress or not self.__rpc_register_queue.empty()) and self.wt_client.is_connected():
                    ...
                    try:
                        ...
                        log.exception(e)
                        break
                if not self.__request_config_after_connect and self.wt_client.is_connected() and not self.wt_client.client.get_subscriptions_in_progress():
                    self.__request_config_after_connect = True
                    self.__check_shared_attributes()

                if cur_time - gateway_statistic_send > 5000.0 and self.wt_client.is_connected():
        ...
        try:
            ...
            if content is not None:
                ...
                if new_configuration is not None and self.__remote_configurator is not None:
                    try:
                        confirmed = self.__remote_configurator.wt_process_configuration(new_configuration)
                        # if confirmed:
                            # self._send_thread = Thread(target=self.__read_data_from_storage, daemon=True,
                            #                            name="Send data to Waveletthings Thread")
                            # self._send_thread.start()
    ...
    def _load_connectors(self, main_config):
        ...
        if main_config.get("connectors"):
            for connector in main_config['connectors']:
                try:
                    connector_class = WTUtility.check_and_import(connector["type"], self._default_connectors.get(connector["type"], connector.get("class")))
    ...
    def send_to_storage(self, connector_name, data):
        if not connector_name == self.name:
            ...
            data["deviceName"] = "currentWaveletThingsGateway"
                ...
                if self.wt_client.is_connected():
                    ...
                    if events:
                        ...
                        if devices_data_in_event_pack:
                            if not self.wt_client.is_connected():
                                continue
                            while self.__rpc_reply_sent:
                                sleep(.01)
                            self.__send_data(devices_data_in_event_pack)
                        if self.wt_client.is_connected() and (self.__remote_configurator is None or not self.__remote_configurator.in_process):
                            success = True
                            while not self._published_events.empty():
                                if (self.__remote_configurator is not None and self.__remote_configurator.in_process) or \
                                        not self.wt_client.is_connected() or \
        ...
        try:
            for device in devices_data_in_event_pack:
                if devices_data_in_event_pack[device].get("attributes"):
                    if device == self.name or device == "currentWaveletThingsGateway":
                        self._published_events.put(self.wt_client.client.send_attributes(devices_data_in_event_pack[device]["attributes"]))
                    else:
                        self._published_events.put(self.wt_client.client.gw_send_attributes(device, devices_data_in_event_pack[device]["attributes"]))
                if devices_data_in_event_pack[device].get("telemetry"):
                    if device == self.name or device == "currentWaveletThingsGateway":
                        self._published_events.put(self.wt_client.client.send_telemetry(devices_data_in_event_pack[device]["telemetry"]))
                    else:
                        self._published_events.put(self.wt_client.client.gw_send_telemetry(device, devices_data_in_event_pack[device]["telemetry"]))
    ...
    def send_rpc_reply(self, device=None, req_id=None, content=None, success_sent=None, wait_for_publish=None, quality_of_service=0):
        try:
            ...
            if device is not None and success_sent is not None:
                self.wt_client.client.gw_send_rpc_reply(device, req_id, dumps(rpc_response), quality_of_service=quality_of_service)
            elif device is not None and req_id is not None and content is not None:
                self.wt_client.client.gw_send_rpc_reply(device, req_id, content, quality_of_service=quality_of_service)
            elif device is None and success_sent is not None:
                self.wt_client.client.send_rpc_reply(req_id, dumps(rpc_response), quality_of_service=quality_of_service, wait_for_publish=wait_for_publish)
            elif device is None and content is not None:
                self.wt_client.client.send_rpc_reply(req_id, content, quality_of_service=quality_of_service, wait_for_publish=wait_for_publish)
            self.__rpc_reply_sent = False
        except Exception as e:
            log.exception(e)
            ...
            for device_name in devices:
                try:
                    if self.available_connectors.get(devices[device_name]):
                        ...
                        log.info("Pair device {device_name} - connector {connector} from persistent device storage - not found.".format(device_name=device_name, connector=devices[device_name]))
                except Exception as e:
                    log.exception(e)
                    continue
        else:
...
if __name__ == '__main__':
    WTGatewayService(path.dirname(path.dirname(path.abspath(__file__))) + '/config/wt_gateway.yaml'.replace('/', path.sep))
